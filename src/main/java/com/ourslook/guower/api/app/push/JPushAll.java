...
public class JPushAll {
    ...
    public static int sendToRegistrationId( String registrationId,String notification_title, String msg_title, String msg_content, String id, String type, int days) {
        int result = 0;
        try {
            PushPayload pushPayload= JPushAll.buildPushObject_all_registrationId_alertWithTitle(registrationId,notification_title,msg_title,msg_content,id,type,days);
     ...
     * @return 0推送失败，1推送成功
     */
    public static int sendToAllByAlias(List<String> list, String notification_title, String msg_title, String msg_content, String id, String type, int days) {
        int result = 0;
        try {
            PushPayload pushPayload= JPushAll.buildPushObject_all_alias_alertWithTitle(list, notification_title,msg_title,msg_content,id,type,days);
            PushResult pushResult=jPushClient.sendPush(pushPayload);
            if(pushResult.getResponseCode()==200){
                result=1;
            }
    ...
    public static int sendToAll( String notification_title, String msg_title, String msg_content, String id, String type, int days) {
        int result = 0;
        try {
            PushPayload pushPayload= JPushAll.buildPushObject_android_and_ios(notification_title,msg_title,msg_content,id,type,days);
            //System.out.println(pushPayload);
            PushResult pushResult=jPushClient.sendPush(pushPayload);
            //System.out.println(pushResult);
            if(pushResult.getResponseCode()==200){
    ...
    private static PushPayload buildPushObject_all_alias_alertWithTitle(List<String> list, String notification_title, String msg_title, String msg_content, String id, String type, int days) {
        return PushPayload.newBuilder()
                //指定要推送的平台，all代表当前应用配置了的所有平台，也可以传android等具体平台
                .setPlatform(Platform.all())
                //指定推送的接收对象，all代表所有人，也可以指定已经设置成功的tag或alias或该应应用客户端调用接口获取到的registration id
