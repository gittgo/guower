...
//
//    @Autowired
//    protected com.ourslook.guower.SpringBootMyApplication springBootMyApplication;
//
//    @Lazy
//    @Autowired
...
//        setCasServerUrlPrefix(springBootMyApplication.getCasServerUrlPrefix());
//        // 客户端回调地址
//        setCasService(springBootMyApplication.getCasclientCasService());
//    }
//
//    /**
...
//        //获取当前登录输入的用户名，等价于(String) principalCollection.fromRealm(getFilename()).iterator().next();
//        String loginName = (String) super.getAvailablePrincipal(principalCollection);
//        //到数据库查是否有此对象
//        // 实际项目中，这里可以根据实际情况做缓存，如果不做，Shiro自己也是有时间间隔机制，2分钟内不会重复执行该方法
//        SysUserEntity user = userDao.queryByUserName(loginName);
//        if (user != null) {
//            //权限信息对象info,用来存放查出的用户的所有的角色（role）及权限（permission）
//            SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
//            //用户的角色集合
//            //用户权限列表
//            Set<String> permsSet = sysMenuService.getUserPermissions(user.getUserId());
