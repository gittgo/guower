...
public class HttpsRequest implements IServiceRequest {
    ...
    private void init() throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        ...
        File certFile = ResourceUtils.getFile(WxpayConfig.getHttpsCertLocalPath());
        FileInputStream instream = new FileInputStream(certFile);//加载本地的证书进行https加密传输
        try {
            keyStore.load(instream, WxpayConfig.getHttpsCertPassword().toCharArray());//设置证书密码
        ...
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, WxpayConfig.getHttpsCertPassword().toCharArray())
                .build();
    ...
    public String send(String url, Object data) throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        httpClient = HttpClients.custom()
                .build();
        //根据默认超时限制初始化requestConfig
        requestConfig = RequestConfig.custom().setSocketTimeout(socketTimeout).setConnectTimeout(connectTimeout).build();
        ...
        XStream xStreamForRequestPostData = new XStream(new DomDriver("UTF-8", new XmlFriendlyNameCoder("-_", "_")));
        //将要提交给API的数据对象转换成XML格式数据Post给API
        String postDataXML = data.toString();
        Util.log("API，POST过去的数据是：");
        Util.log(postDataXML);
        //得指明使用UTF-8编码，否则到API服务器XML的中文不能被成功识别
        StringEntity postEntity = new StringEntity(postDataXML, "UTF-8");
    ...
    public String sendPostNoSSL(String url, Object xmlObj) throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        ...
        try {
            ...
            log.e("http no  reponse throw Exception");
