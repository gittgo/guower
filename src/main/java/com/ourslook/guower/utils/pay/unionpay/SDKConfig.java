...
public class SDKConfig {
    ...
    /** version，没配按5.0.0 acpsdk.version  */
        ...
        if (StringUtils.isNotBlank(rootPath)) {
            ...
            if (file.exists()) {
                try {
                    ...
                    /**
                     * @author dazer 添加，在这里处理certs整数的位置
                     * 处理三个cer的位置
                     *
                     * acp_test_enc.cer     SDK_ENCRYPTCERT_PATH = "acpsdk.encryptCert.path";  this.encryptCertPath
                     * acp_test_middle.cer  SDK_MIDDLECERT_PATH  = "acpsdk.middleCert.path";   this.middleCertPath
                     * acp_test_root.cer    SDK_ROOTCERT_PATH    = "acpsdk.rootCert.path";     this.rootCertPath     D:/certs/acp_prod_root.cer
                     * acp_test_sign.pfx    SDK_SIGNCERT_PATH    = "acpsdk.signCert.path";     this.signCertPath     D:/certs/从cfca获取到的私钥证书.pfx
                     */
                    if (rootPath.endsWith("dev")) {
                        this.encryptCertPath = this.encryptCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                        this.middleCertPath = this.middleCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                        this.rootCertPath = this.rootCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                        this.signCertPath = this.signCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                    } else if (rootPath.endsWith("prod"))  {
                        this.encryptCertPath = this.encryptCertPath.toLowerCase().toLowerCase().replace("d:/certs/", rootPath+File.separator);
                        this.middleCertPath = this.middleCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                        this.rootCertPath = this.rootCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                        this.signCertPath = this.signCertPath.toLowerCase().replace("d:/certs/", rootPath+File.separator);
                    }
