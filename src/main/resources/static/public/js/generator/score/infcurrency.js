$(function () {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#jqGrid'
            , url: getWebPath() + 'infcurrency/list'
            //, width: 1100
            //, height: 488
            , cellMinWidth: 80
            ...
            , cols: [[
                ...
                , {field: 'currencyName', minWidth: 120, title: '货币名称', align: 'center', sort: true}
                // , {field: 'image', minWidth: 120, title: '图标', align: 'center', sort: true}
                , {field: 'score', minWidth: 120, title: '积分价格', align: 'center', sort: true}
                , {field: 'count', minWidth: 120, title: '库存', align: 'center', sort: true}
                ...
                // , {field: 'sortTime', minWidth: 120, title: '排序时间', align: 'center', sort: true}
                ]]
            , page: true
            , limits: [10, 30, 50, 100],
            response: {
    ...
    var vm = new Vue({
        ...
        methods: {
            ...
            saveOrUpdate: function (event) {
                vm.infCurrency.image = $("#imgUrl").val();
                $('#addOrUpdateForm').isValid(function (result) {
                    if (!result) {
                        ...
                        subClick(event);
                        showLoading(event);
                        var url = vm.infCurrency.id == null ? "infcurrency/save" : "infcurrency/update";
                        $.ajax({
                            type: "POST",
                            url: getWebPath() + url,
                            data: JSON.stringify(vm.infCurrency),
                            success: function(r){
                                changeClick(event);
                                closeLoading(event);
            ...
            del: function (event) {
                ...
                confirm('确定要删除选中的记录？', function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: getWebPath() + "infcurrency/delete",
            ...
            operateByIds: function(status) {
                ...
                confirm(tips, function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: getWebPath() + "infcurrency/operateInfCurrencyByIds",
            ...
            exportsAllToExcel:function (isCvs) {
                ...
                location.href = getWebPath() + 'infcurrency/exportsExcelAll?' + data;
            },
            getInfo: function(id){
                //获取详情信息
                $.get(getWebPath() + "infcurrency/info/"+id, function(r){
                    vm.infCurrency = r.infCurrency;
                    $("#myImageShow").attr("src",vm.infCurrency.image);
                    $("#imgUrl").val(vm.infCurrency.image);
                });
            },
            goBack: function () {
                //详情返回列表
            ...
            reload: function (sortObj) {
                ...
                table.reload('jqGrid', {
                    ...
                    , where: {//设定异步数据接口的额外参数，任意设
                        ...
                        'currencyName': vm.q.currencyName,
