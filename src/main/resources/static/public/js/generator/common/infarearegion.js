$(function () {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#jqGrid'
            , url: getWebPath() + 'infarearegion/list'
            //, height: 488
            , cellMinWidth: 80
            , cols: [[
                {type: 'numbers', fixed: 'left'}
                , {type: 'checkbox', fixed: 'left'}
                                                            , {field: 'regionId', width: 120, title: 'regionId', align: 'center', sort: true}
                                                                                , {field: 'regionName', width: 120, title: '区域名称', align: 'center', sort: true}
                                                                                , {field: 'cityId', width: 120, title: '城市id', align: 'center', sort: true}
	...
	methods: {
		...
		saveOrUpdate: function (event) {
            $('#addOrUpdateForm').isValid(function (result) {
                if (!result) {
                    ...
                    subClick(event);
                    var url = vm.infArearegion.regionId == null ? "infarearegion/save" : "infarearegion/update";
                    $.ajax({
                        type: "POST",
                        url: getWebPath() + url,
                        data: JSON.stringify(vm.infArearegion),
                        success: function(r){
                            changeClick(event);
                            if(r.code === 0){
		...
		del: function (event) {
			...
			for (var i = 0; i < selectRows.length; ++i) {
                ids.push(selectRows[i].Region_Id);
            }

			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: getWebPath() + "infarearegion/delete",
        ...
        exportsToExcel:function () {
            ...
            //导出选中项
                ...
                for (var i= 0; i < ids.length; ++i) {
                    idss.push(ids[i].Region_Id);
                }
                var modelIds = idss.join(",");
                location.href = getWebPath() + 'infarearegion/exportsExcel?modelIds='  + modelIds;
            }
        },
        exportsAllToCvs:function () {
            ...
            location.href = getWebPath() + 'infarearegion/exportsExcelAll?' + data;
        },
		...
		getInfo: function(regionId){
			$.get(getWebPath() + "infarearegion/info/"+regionId, function(r){
                vm.infArearegion = r.infArearegion;
            });
		},
