$(function () {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#jqGrid'
            , url: getWebPath() + 'infareaprovince/list'
            //, height: 488
            , cellMinWidth: 80
            , cols: [[
                {type: 'numbers', fixed: 'left'}
                , {type: 'checkbox', fixed: 'left'}
                                                            , {field: 'provinceId', width: 120, title: 'provinceId', align: 'center', sort: true}
                                                                                , {field: 'provinceName', width: 120, title: '省份名称', align: 'center', sort: true}
...
var vm = new Vue({
	...
	methods: {
		...
		update: function (event) {
            ...
            vm.getInfo(selectRow.provinceId);
            // nice-validate 清空表单验证消息
            $('#addOrUpdateForm').validator("cleanUp");
		},
		saveOrUpdate: function (event) {
            $('#addOrUpdateForm').isValid(function (result) {
                if (!result) {
                    ...
                    subClick(event);
                    var url = vm.infAreaprovince.provinceId == null ? "infareaprovince/save" : "infareaprovince/update";
                    $.ajax({
                        type: "POST",
                        url: getWebPath() + url,
                        data: JSON.stringify(vm.infAreaprovince),
                        success: function(r){
                            changeClick(event);
                            if(r.code === 0){
		...
		del: function (event) {
			...
			for (var i = 0; i < selectRows.length; ++i) {
                ids.push(selectRows[i].Province_id);
            }

			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: getWebPath() + "infareaprovince/delete",
        ...
        exportsToExcel:function () {
            ...
            //导出选中项
                ...
                for (var i= 0; i < ids.length; ++i) {
                    idss.push(ids[i].Province_id);
                }
                var modelIds = idss.join(",");
                location.href = getWebPath() + 'infareaprovince/exportsExcel?modelIds='  + modelIds;
            }
        },
        exportsAllToCvs:function () {
            ...
            location.href = getWebPath() + 'infareaprovince/exportsExcelAll?' + data;
        },
		...
		getInfo: function(provinceId){
			$.get(getWebPath() + "infareaprovince/info/"+provinceId, function(r){
                vm.infAreaprovince = r.infAreaprovince;
            });
		},
