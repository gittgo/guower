$(function () {
    ...
    layui.use('table', function () {
        ...
        table.render({
            elem: '#jqGrid'
            , url: getWebPath() + 'infscoredaymax/list'
            //, width: 1100
            //, height: 488
            , cellMinWidth: 80
            ...
            , cols: [[
                ...
                , {field: 'getType', minWidth: 150, title: '获取途径', align: 'center', sort: true,templet : function(data){
                        if(data.getType == 1){
                            return '注册';
                ...
                , {field: 'getNumber', minWidth: 150, title: '单次获取量', align: 'center', sort: true}
                , {field: 'getMax', minWidth: 150, title: '单日获取上限', align: 'center', sort: true}
                ]]
            , page: true
            , limits: [10, 30, 50, 100],
            response: {
        ...
        methods: {
            ...
            saveOrUpdate: function (event) {
                $('#addOrUpdateForm').isValid(function (result) {
                    if (!result) {
                        ...
                        subClick(event);
                        showLoading(event);
                        var url = vm.infScoreDayMax.id == null ? "infscoredaymax/save" : "infscoredaymax/update";
                        $.ajax({
                            type: "POST",
                            url: getWebPath() + url,
                            data: JSON.stringify(vm.infScoreDayMax),
                            success: function(r){
                                changeClick(event);
                                closeLoading(event);
            ...
            del: function (event) {
                ...
                confirm('确定要删除选中的记录？', function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: getWebPath() + "infscoredaymax/delete",
            ...
            operateByIds: function(status) {
                ...
                confirm(tips, function(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: getWebPath() + "infscoredaymax/operateInfScoreDayMaxByIds",
            ...
            exportsAllToExcel:function (isCvs) {
                ...
                location.href = getWebPath() + 'infscoredaymax/exportsExcelAll?' + data;
            },
            getInfo: function(id){
                //获取详情信息
                $.get(getWebPath() + "infscoredaymax/info/"+id, function(r){
                    vm.infScoreDayMax = r.infScoreDayMax;
                });
            },
            goBack: function () {
                //详情返回列表
                ...
                table.reload('jqGrid', {
                    ...
                    , where: {//设定异步数据接口的额外参数，任意设
                        ...
                        //这里是所有的筛选条件，没用的请自行删除，不用注释
                                                    'id': vm.q.id,
                                                    'getType': vm.q.getType,
                                                    'getNumber': vm.q.getNumber,
                                                    'getMax': vm.q.getMax,
                                                //end
                        sidx: aSortName //排序字段
